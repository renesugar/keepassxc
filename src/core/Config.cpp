...
void Config::upgrade()
{
    ...
    if (previousVersion < 1) {
        ...
        if (m_localSettings) {
            for (const auto& setting : configStrings) {
    ...
    : QObject(parent)
{
    ...
    localConfigPath =
        QDir::fromNativeSeparators(QStandardPaths::writableLocation(QStandardPaths::AppLocalDataLocation));
#endif
    ...
    init(configPath, localConfigPath);
}
...
void Config::init(const QString& configFileName, const QString& localConfigFileName)
{
    ...
    if (!localConfigFileName.isEmpty() && QFile::exists(localConfigFileName) && !QFile::exists(configFileName)) {
        ...
        QDir().rmdir(localConfigFileName);
    }
